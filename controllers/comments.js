...
import { Comment, Location, User } from '../models/Schemas.js'
...
export const createComment = async (req, res) => {
  try {
    ...
    const updateUser = await User.findOneAndUpdate(
      { _id: user_id },
      { $push: { comments_list: newComment._id } },
    )
...
export const updateComment = async (req, res) => {
  try {
    ...
    const updatedComment = await Comment.findOneAndUpdate(
      ...
      .populate({ path: 'flavors_referred', select: 'name' })
...
export const deleteComment = async (req, res) => {
  try {
    ...
    if (singleComment.user_id != user_id)
      return res.status(403).json({ message: 'You are not allowed to delete this comment' })

    // delete comment id also in the refered User collection and Location collection
    await User.findOneAndUpdate({ _id: singleComment.user_id }, { $pull: { comments_list: id } })
